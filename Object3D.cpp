...
void Object3D::TranslateAllWorldPoints()
{
	for (int i = 0; i < worldSpacePoints3D.size(); i++)
	{
		...
		/*worldSpacePoints3D[i]._x += translationX;
		worldSpacePoints3D[i]._y += translationY;
		worldSpacePoints3D[i]._y += translationZ;*/
...
void Object3D::Render(SDL_Renderer* renderer)
{
	for (Vertices ver : vertices)
	{
		...
		/*startPoint3D = modelSpacePoints3D[ver._start];
		endPoint3D = modelSpacePoints3D[ver._end];
		...
		// speed
		translationX += speedX * 10;
		translationY += speedY * 10;
		translationZ += speedZ * 10;
...
void Object3D::TranslateXYZ(double x, double y, double z)
{
	for (int i = 0; i < modelSpacePoints3D.size(); i++)
	{
		//modelSpacePoints3D[i].TranslateXYZ(x, y, z);

		translationX += x;
		translationY += y;
		translationZ += z;

		//modelSpacePoints3D[i].ShowXYZ();
	}
}